    ...
    public static function render_menu($mode = 'text', $order_by = 'normal', $top_level_only = FALSE) {
        ...
        foreach ($contexts as $context) {
            if (has_permission('Site.' . ucfirst($context) . '.View') == true || permission_exists('Site.' . ucfirst($context) . '.View') == false) {
                ...
                $nav .= "<li class='dropdown {$class}'><a href='{$url}' id='{$id}' class='dropdown-toggle special_menu' title='{$title}' data-toggle='dropdown' data-id='{$context}_menu'>";
    ...
    public function context_nav($context = NULL, $class = 'dropdown-menu', $ignore_ul = FALSE) {
        ...
        foreach ($module_list as $module) {
            if (module_controller_exists($context, $module) === TRUE) {
                ...
                //-----------------------------------------BOB-----------------------------------------------
                //-------------------- add by datls for support menu multi language -------------------------
                $display_name = $module;
                if (isset($mod_config['name'])) {
                    if (is_array($mod_config['name'])) {
                        if (isset($mod_config['name'][CI::$APP->config->item('language')]))
                            $display_name = $mod_config['name'][CI::$APP->config->item('language')];
                        else if (isset($mod_config['name']['english']))
                            $display_name = $mod_config['name']['english'];
                    }
                    else {
                ...
                if (isset($mod_config['description'])) {
                    if (is_array($mod_config['description'])) {
                        if (isset($mod_config['description'][CI::$APP->config->item('language')]))
                            $title = $mod_config['description'][CI::$APP->config->item('language')];
                        else if (isset($mod_config['description']['english']))
                            $title = $mod_config['description']['english'];
                    }
                    else {
                        $title = $mod_config['description'];
                    }
                }
                //-------------------- add by datls for support menu multi language -------------------------
                //-----------------------------------------EOB-----------------------------------------------
                ...
                self::$actions[$module] = array(
                    ...
                    'display_name' => $display_name, //isset($mod_config['name']) ? $mod_config['name'] : $module,
                    'title' => $title, //isset($mod_config['description']) ? $mod_config['description'] : $module,
...
//end build_item()
    ...
    private function sort_actions() {
        ...
        array_multisort($weights, SORT_ASC, $display_names, SORT_ASC, self::$actions);
