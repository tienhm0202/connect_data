...
?>
<div class="admin-box">
	<h3><?php echo lang('books_action_edit') ?></h3>
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
			...
			<div class="control-group <?php echo form_error('title') ? 'error' : ''; ?>">
				...
				<div class='controls'>
					<textarea id='description' rows="10" class="span9" name='description' maxlength="50"><?php echo set_value('description', isset($books['description']) ? $books['description'] : ''); ?></textarea>
			...
			<div class="control-group <?php echo form_error('published') ? 'error' : ''; ?>">
				...
				<div class='controls' aria-labelled-by='books_published_label'>
					<label class='radio' for='books_published_option1'>
						<input id='books_published_option1' name='books_published' type='radio' class='' value='Y' <?php echo set_radio('books_published', 'Y', $books['published'] == 'Y' ? TRUE : FALSE); ?> />
						<?php echo lang('yes') ?>
					</label>
					<label class='radio' for='books_published_option2'>
						<input id='books_published_option2' name='books_published' type='radio' class='' value='N' <?php echo set_radio('books_published', 'N',$books['published'] == 'N' ? TRUE : FALSE); ?> />
						<?php echo lang('no') ?>
					</label>
			...
			<div class="form-actions">
				<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('books_action_edit'); ?>"  />
				<?php echo lang('bf_or'); ?>
			...
			<?php if ($this->auth->has_permission('Books.Content.Delete')) : ?>
				<?php echo lang('bf_or') ?>
				<button type="submit" name="delete" class="btn btn-danger" id="delete-me" onclick="return confirm('<?php e(js_escape(lang('books_delete_confirm'))); ?>'); ">
					<span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('books_delete_record'); ?>
				</button>
			<?php endif; ?>
			</div>
		</fieldset>
