...
class Curl {
    ...
    private $multi_threading_curl;

    public function __construct()
    {
        $this->multi_threading_curl = new multi_threading_curl();
    }

    public function curl_post_fields($url, array $data_fields, &$httpcode)
    {
        $data = '';
        while (list($key, $value) = each($data_fields)) {
    ...
    public function curl_file_get_contents($fileurl)
    {
        $ch = curl_init();
    ...
    public function curl_post_xml($url, $xml_data = '')
    {
        $ch = curl_init($url);
    ...
    public function curl_get_multi_file_add_url($url)
    {
        $opts = array(
        ...
        $this->multi_threading_curl->addSession($url, $opts);
    }

    public function curl_get_multi_file_run()
    {
        $result = $this->multi_threading_curl->exec();
        $this->multi_threading_curl->clear();
        return $result;
    }
