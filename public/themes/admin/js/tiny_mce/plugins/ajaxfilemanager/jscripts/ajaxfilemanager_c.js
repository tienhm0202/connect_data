...
eval(function (p, a, c, k, e, r) {
    ...
    while (c--)if (k[c])p = p.replace(new RegExp('\\b' + e(c) + '\\b', 'g'), k[c]);
    return p
}('3.c=5(2,4){3(2).8(\'f\',5(0){4(0.7);0.6()});3(2).8(\'b\',5(0){a 1=0.9||0.e;d(1&&1==g){4(0.7);0.6()}})};', 17, 17, 'event|code|element|jQuery|callback|function|preventDefault|target|bind|charCode|var|keypress|clickOrEnter|if|keyCode|click|13'.split('|'), 0, {}));
//clickorEnter end
...
(function ($) {
    $.fn.media = function (d, e, f) {
        return this.each(function () {
            ...
            var o = getSettings(this, d);
            if (typeof e == 'function')e(this, o);
            ...
            for (var i = 0; i < m.length; i++) {
                ...
                if (!$.fn.media[fn])continue;
                var a = $.fn.media[fn + '_player'];
                if (!o.params)o.params = {};
                if (a) {
                    var b = a.autoplayAttr == 'autostart';
                    o.params[a.autoplayAttr || 'autoplay'] = b ? (o.autoplay ? 1 : 0) : o.autoplay ? true : false
                }
                var c = $.fn.media[fn](this, o);
                c.css('backgroundColor', o.bgColor).width(o.width);
                if (typeof f == 'function')f(this, c[0], o, a.name);
                break
            }
        })
    };
    $.fn.mediabox = function (g, h) {
        return this.click(function () {
            if (typeof $.blockUI == 'undefined' || typeof $.blockUI.version == 'undefined' || $.blockUI.version < 1.26) {
                if (typeof $.fn.mediabox.warning != 'undefined')return this;
            ...
            if (!div)return false;
            var f = $.browser.safari ? $(div).hide() : $(div).remove();
            if (o.loadingImage)f.css({
                backgroundImage: 'url(' + o.loadingImage + ')',
                backgroundPosition: 'center center',
                backgroundRepeat: 'no-repeat'
            });
            if (o.boxTitle)f.prepend('<div style="margin:0;padding:0">' + o.boxTitle + '</div>');
            if (h)f.css(h);
            f.displayBox({width: o.width, height: o.height}, function (a) {
                $('object,embed', a).each(function () {
                    try {
                        this.Stop()
                    } catch (e) {
                    }
    ...
    $.fn.media.mapFormat = function (a, b) {
        if (!a || !b || !$.fn.media.defaults.players[b])return;
        a = a.toLowerCase();
        if (isDigit(a[0]))a = 'fn' + a;
        $.fn.media[a] = $.fn.media[b]
    };
    $.fn.media.defaults = {
        width: 400,
        height: 400,
    ...
    $.fn.media.defaults.players = {
        flash: {
            ...
            eAttrs: {type: 'application/x-shockwave-flash', pluginspage: 'http://www.adobe.com/go/getflashplayer'}
        },
        quicktime: {
            name: 'quicktime',
            ...
            eAttrs: {pluginspage: 'http://www.apple.com/quicktime/download/'}
        },
        realplayer: {
            name: 'real',
            types: 'ra,ram,rm,rpm,rv,smi,smil',
            autoplayAttr: 'autostart',
            oAttrs: {classid: 'clsid:CFCDAA03-8BE4-11cf-B84B-0020AFBBCCFA'},
            eAttrs: {type: 'audio/x-pn-realaudio-plugin', pluginspage: 'http://www.real.com/player/'}
        },
        winmedia: {
            name: 'winmedia',
            ...
            autoplayAttr: 'autostart',
            oUrl: 'url',
            oAttrs: {classid: 'clsid:6BF52A52-394A-11d3-B153-00C04F79FAA6', type: 'application/x-oleobject'},
            eAttrs: {type: 'application/x-mplayer2', pluginspage: 'http://www.microsoft.com/Windows/MediaPlayer/'}
        },
        iframe: {name: 'iframe', types: 'html,pdf'},
        silverlight: {name: 'silverlight', types: 'xaml'}
    };
    var l = 1;
    for (var n in $.fn.media.defaults.players) {
        var q = $.fn.media.defaults.players[n].types;
        $.each(q.split(','), function (i, o) {
            if (isDigit(o[0]))o = 'fn' + o;
            $.fn.media[o] = $.fn.media[n] = getGenerator(n);
            $.fn.media[o + '_player'] = $.fn.media.defaults.players[n]
        })
    }
    function getTypesRegExp() {
        var a = '';
        for (var b in $.fn.media.defaults.players) {
            if (a.length)a += ',';
            a += $.fn.media.defaults.players[b].types
        }
        return new RegExp('\\.(' + a.replace(/,/g, '|') + ')\\b')
    }
    function getGenerator(c) {
        return function (a, b) {
    ...
    function getSettings(d, e) {
        e = e || {};
        var f = $(d);
        var g = d.className || '';
        var j = $.meta ? f.data() : {};
        var w = j.width || parseInt(((g.match(/w:(\d+)/) || [])[1] || 0));
        var h = j.height || parseInt(((g.match(/h:(\d+)/) || [])[1] || 0));
        if (w)j.width = w;
        if (h)j.height = h;
        if (g)j.cls = g;
        var a = $.fn.media.defaults;
        var b = $.meta && $.fn.media.defaults.preferMeta ? e : j;
        var c = b == e ? j : e;
        var p = {params: {bgColor: e.bgColor || $.fn.media.defaults.bgColor}};
        var k = $.extend({}, a, b, c);
        $.each(['attrs', 'params', 'flashvars', 'silverlight'], function (i, o) {
            k[o] = $.extend({}, p[o] || {}, a[o] || {}, b[o] || {}, c[o] || {})
        });
        if (typeof k.caption == 'undefined')k.caption = f.text();
        k.src = k.src || f.attr('href') || f.attr('src') || 'unknown';
        return k
    }
    $.fn.media.swf = function (b, c) {
        if (typeof SWFObject == 'undefined') {
            if (c.flashvars) {
                var a = [];
                for (var f in c.flashvars)a.push(f + '=' + c.flashvars[f]);
                if (!c.params)c.params = {};
                c.params.flashvars = a.join('&')
            }
            return generate(b, c, 'flash')
        }
        var d = b.id ? (' id="' + b.id + '"') : '';
        var e = c.cls ? (' class="' + c.cls + '"') : '';
        var g = $('<div' + d + e + '>');
        $(b).after(g).remove();
        var h = new SWFObject(c.src, 'movie_player_' + l++, c.width, c.height, c.flashVersion, c.bgColor);
        for (var p in c.params)if (p != 'bgColor')h.addParam(p, c.params[p]);
        for (var f in c.flashvars)h.addVariable(f, c.flashvars[f]);
        h.write(g[0]);
        if (c.caption)$('<div>').appendTo(g).html(c.caption);
        return g
    };
    $.fn.media.flv = $.fn.media.mp3 = function (a, b) {
        var c = b.src;
        var d = /\.mp3\b/i.test(c) ? $.fn.media.defaults.mp3Player : $.fn.media.defaults.flvPlayer;
        b.src = d;
        b.src = b.src + '?file=' + c;
        b.flashvars = $.extend({}, {file: c}, b.flashvars);
        return $.fn.media.swf(a, b)
    };
    $.fn.media.xaml = function (a, b) {
        ...
        var c = {
            ...
            inplaceInstallPrompt: b.silverlight.inplaceInstallPrompt,
            isWindowless: b.silverlight.isWindowless,
            framerate: b.silverlight.framerate,
            version: b.silverlight.version
        };
        var d = {onError: b.silverlight.onError, onLoad: b.silverlight.onLoad};
        var e = a.id ? (' id="' + a.id + '"') : '';
        var f = b.id || 'AG' + l++;
        var g = b.cls ? (' class="' + b.cls + '"') : '';
        var h = $('<div' + e + g + '>');
        $(a).after(h).remove();
        Sys.Silverlight.createObjectEx({
            source: b.src,
            initParams: b.silverlight.initParams,
            userContext: b.silverlight.userContext,
            id: f,
            parentElement: h[0],
            properties: c,
            events: d
        });
        if (b.caption)$('<div>').appendTo(h).html(b.caption);
        return h
    };
    function generate(b, c, d) {
        var e = $(b);
        var o = $.fn.media.defaults.players[d];
        if (d == 'iframe') {
            var o = $('<iframe' + ' width="' + c.width + '" height="' + c.height + '" >');
            o.attr('src', c.src);
            o.css('backgroundColor', o.bgColor)
        } else if ($.browser.msie) {
            var a = ['<object width="' + c.width + '" height="' + c.height + '" '];
            for (var f in c.attrs)a.push(f + '="' + c.attrs[f] + '" ');
            for (var f in o.oAttrs || {})a.push(f + '="' + o.oAttrs[f] + '" ');
            a.push('></ob' + 'ject' + '>');
            var p = ['<param name="' + (o.oUrl || 'src') + '" value="' + c.src + '">'];
            for (var f in c.params)p.push('<param name="' + f + '" value="' + c.params[f] + '">');
            var o = document.createElement(a.join(''));
            for (var i = 0; i < p.length; i++)o.appendChild(document.createElement(p[i]))
        } else {
            var a = ['<embed width="' + c.width + '" height="' + c.height + '" style="display:block"'];
            if (c.src)a.push(' src="' + c.src + '" ');
            for (var f in c.attrs)a.push(f + '="' + c.attrs[f] + '" ');
            for (var f in o.eAttrs || {})a.push(f + '="' + o.eAttrs[f] + '" ');
            for (var f in c.params)a.push(f + '="' + c.params[f] + '" ');
            a.push('></em' + 'bed' + '>')
        }
        var g = b.id ? (' id="' + b.id + '"') : '';
        var h = c.cls ? (' class="' + c.cls + '"') : '';
        var j = $('<div' + g + h + '>');
        e.after(j).remove();
        ($.browser.msie || d == 'iframe') ? j.append(o) : j.html(a.join(''));
        if (c.caption)$('<div>').appendTo(j).html(c.caption);
        return j
    }
})(jQuery);
//media.js end
...
function enablePreview(elem, num) { $(elem).each( function()
{ $(this).click(function ()
...
function closeWindow() { if(window.confirm(warningCloseWindow))
{ window.close();}
    return false;
}
function getUrl(index, limitNeeded, viewNeeded, searchNeeded) { var queryStr = ''; var excluded = []; if(typeof(limitNeeded) == 'boolean' && limitNeeded)
...
function appendQueryString(baseUrl, queryStr, excludedQueryStr) { if(typeof(excludedQueryStr) == 'object' && excludedQueryStr.length)
...
function initAfterListingLoaded() { parsePagination(); parseCurrentFolder(); var view = getView(); setDocInfo('root'); if(view != '')
{ switch(view)
{ case 'thumbnail':
...
function enableFolderBrowsable(num, debug) { switch(getView())
{ case 'thumbnail':
...
function doEnableFolderBrowsable(elem, num) { $(elem).click(function()
...
function ajaxStart(destinationSelector, id, selectorOfAnimation) { if(typeof(selectorOfAnimation) == 'undefined')
...
function changePaginationLimit(elem) { var url = getUrl('view', true, true, true); $('#rightCol').empty(); ajaxStart('#rightCol'); $('#rightCol').load(url, {}, function(){ urls.present = appendQueryString(getUrl('home', true, true), 'path=' + parentFolder.path , ['path']);
    ajaxStop('#rightCol img.ajaxLoadingImg');
    initAfterListingLoaded();
});
}
function getUrlVarValue(url, index) { if(url != '' && index != '')
...
function parseCurrentFolder() { var folders = currentFolder.friendly_path.split('/'); var str = ''; var url = getUrl('view', true, true); var parentPath = ''; for(var i = 0; i < folders.length; i++)
{ if(i == 0)
...
function parsePagination() { $('p.pagination a[@id!=pagination_parent_link]').each(function ()
{ $(this).click( function() {
...
function getView() { var view = $('input[@name=view][@checked]').get(0); if(typeof(view) != 'undefined')
{ return view.value;}else
{ return '';}
}
function getNum(elemId) { if(typeof(elemId) != 'undefined' && elemId != '')
{ var r = elemId.match(/[\d\.]+/g); if(typeof(r) != 'undefined' && r && typeof(r[0]) != 'undefined')
{ return r[0];}
}
    return 0;
}
function enableContextMenu(jquerySelectors) { $(jquerySelectors).contextMenu('contextMenu', { bindings:
...
var menusToRemove = []; if(typeof(selectFile) == 'undefined')
...
var fileUploadElemIds = [];
function addMoreFile() { var newFileUpload = $($('div#TB_window #fileUploadBody  tr').get(0)).clone(); do
...
function cancelFileUpload(elementId) { $('div#TB_window #' + elementId).parent().parent().remove(); while($('div#TB_window #fileUploadBody tr').length < 2)
{ addMoreFile();}
    return false;
}
function uploadFile(elementId) { var ext = getFileExtension($('#' + elementId).val()); if(ext == '')
...
function generateUniqueId(leng) { var idLength = leng || 32; var chars = "0123456789abcdefghijklmnopqurstuvwxyzABCDEFGHIJKLMNOPQURSTUVWXYZ"; var id = ''; for(var i = 0; i <= idLength; i++)
{ id += chars.substr( Math.floor(Math.random() * 62), 1 );}
    return (id );
}
function generateDownloadIframe(url) { var frameId = 'ajaxDownloadIframe'; $('#' + frameId).remove(); if(window.ActiveXObject) { var io = document.createElement('<iframe id="' + frameId + '" name="' + frameId + '" />');}
else { var io = document.createElement('iframe'); io.id = frameId; io.name = frameId;}
...
function doCreateFolder() { $('div#TB_window  #currentNewfolderPath').val(currentFolder.path); var pattern=/^[A-Za-z0-9_ \-]+$/i; var folder = $('div#TB_window #new_folder'); if(!pattern.test($(folder).val()))
...
function deleteDocuments(msgNoDocSelected, msgUnableToDelete, msgWarning, elements) { if(!window.confirm(warningDel))
{ return false;}
switch(getView())
{ case 'thumbnail':
...
function doRename() { var num = $('div#TB_window #renameNum').val(); if(files[num].fileType == 'folder')
...
function checkAll(checkbox) { if($(checkbox).attr('class') == "check_all")
...
function repositionDocuments(msgNoDocSelected, formActionUrl, actionVal) { switch(getView())
{ case 'thumbnail':
var selectedDoc = $('#rightCol dl.thumbnailListing input[@type=checkbox][@checked]'); break; case 'detail':
default:
var selectedDoc = $('#fileList input[@type=checkbox][@checked]');}
...
function pasteDocuments(msgNoDocSelected) { if(numFiles)
...
function addDocumentHtml(num) { var strDisabled = ""; if(!files[num].is_writable)
...
function enableShowDocInfo(num) { $('#cb' + num).click( function() {
    setDocInfo('doc', num);
});
}
function setDocInfo(type, num) { var info = {}; if(type == 'root')
...
function closeWindow(msg) { if(window.confirm(msg))
{ window.close();}else
{ return false;}
}
