...
(function(){
    function s(a,b){
        ...
        var a,b=arguments.length,c={},d=function(a,b){
            var c,h;
            typeof a!=="object"&&(a={});
            for(h in b)b.hasOwnProperty(h)&&(c=b[h],a[h]=c&&typeof c==="object"&&Object.prototype.toString.call(c)!=="[object Array]"&&typeof c.nodeType!=="number"?d(a[h]||{},c):b[h]);return a
            };
    ...
    else if(t(b)&&T(b))for(d in b)a.setAttribute(d,b[d]);return e
    }
    function ia(a){
    return Ha(a)?
    a:[a]
...
function Ba(a,b){
    ...
    function Cb(a,b){
    var c=b||[[Db,[1,2,5,10,20,25,50,100,200,500]],[nb,[1,2,5,10,15,30]],[Ya,[1,2,5,10,15,30]],[Qa,[1,2,3,4,6,8,12]],[sa,[1,2]],[Za,[1,2]],[Ra,[1,2,3,4,6]],[ta,null]],d=c[c.length-1],e=H[d[0]],f=d[1],g;
    for(g=0;g<c.length;g++)if(d=c[g],e=H[d[0]],f=d[1],c[g+1]&&a<=(e*f[f.length-1]+H[c[g+1][0]])/2)break;e===H[ta]&&a<5*e&&(f=[1,2,5]);
...
Xa=function(a,b,c){
    ...
    var a=
    p(a,"%Y-%m-%d %H:%M:%S"),d=new Date(b),e,f=d[pb](),g=d[rb](),h=d[Sa](),i=d[$a](),j=d[ab](),k=M.lang,l=k.weekdays,d=s({
        a:l[g].substr(0,3),
        A:l[g],
        d:Aa(h),
        e:h,
    ...
    for(e in d)for(;a.indexOf("%"+e)!==-1;)a=a.replace("%"+e,typeof d[e]==="function"?d[e](b):d[e]);return c?a.substr(0,1).toUpperCase()+
    a.substr(1):a
    };
...
X&&X.init.call(X,zb);
var ib=F.adapterRun,Vb=F.getScript,oa=F.inArray,n=F.each,sb=F.grep,Wb=F.offset,Na=F.map,J=F.addEvent,ba=F.removeEvent,K=F.fireEvent,Sb=F.washMouseEvent,Ab=F.animate,Wa=F.stop,F={
    enabled:!0,
    x:0,
    y:15,
    style:{
...
attr:function(a,b){
    ...
    [c]||c:c==="strokeWidth"&&(c="stroke-width"),q=w(g,c)||this[c]||0,c!=="d"&&c!=="visibility"&&(q=parseFloat(q));
        ...
        for(c in a)if(j=!1,d=a[c],e=k[c]&&k[c].call(this,d,c),e!==!1){
            ...
            else if(c==="x"&&h==="text")for(e=0;e<g.childNodes.length;e++)f=g.childNodes[e],w(f,"x")===w(g,"x")&&
                w(f,"x",d);
...
css:function(a){
    ...
    if(ra&&!Z)c&&delete a.width,L(this.element,a);
        ...
        for(d in a)e+=d.replace(/([A-Z])/g,f)+":"+a[d]+";";w(b,"style",e)
        }
            ...
            n(m,function(e){
                if(e!==""||m.length===1){
                    ...
                    if(e!==" "&&(n.appendChild(z.createTextNode(e)),p?o.dx=0:o.x=i,w(n,o),!p&&f&&(!Z&&d&&L(n,{
                        display:"block"
                    }),w(n,"dy",l||c.fontMetrics(/px$/.test(n.style.fontSize)?n.style.fontSize:j.fontSize).h,fb&&n.offsetHeight)),b.appendChild(n),p++,k))for(var e=e.replace(/([^\^])-/g,
...
button:function(a,b,c,d,e,f,g,h){
    var i=this.label(a,b,c,null,null,null,null,null,"button"),j=0,k,l,m,o,q,n,a={
        x1:0,
        y1:0,
        x2:0,
        y2:1
...
color:function(a,b,c){
    ...
    if(g){
        ...
        for(m in c)m!=="id"&&o.push(m,c[m]);for(m in j)o.push(j[m]);o=o.join(",");
...
label:function(a,
    ...
    zIndex:1
}),y,V,r=0,da=3,jb=0,Oa,E,G,I,B=0,kb={},w,g=q.attrSetters,A;
J(q,"add",m);
g.width=function(a){
    Oa=a;
    return!1
...
if(!Z&&!$){
    ...
    pathToVML:function(a){
        for(var b=a.length,c=[],d;b--;)if(qa(a[b]))c[b]=u(a[b]*10)-5;
            else if(a[b]==="Z")c[b]=
            "x";
        else if(c[b]=a[b],a.isArc&&(a[b]==="wa"||a[b]==="at"))d=a[b]==="wa"?1:-1,c[b+5]===c[b+7]&&(c[b+7]-=d),c[b+6]===c[b+8]&&(c[b+8]-=d);return c.join(" ")||"x"
        },
    attr:function(a,b){
...
color:function(a,b,c,d){
    ...
    if(i){
        ...
        if(c==="fill")if(i==="gradient")c=m.x1||m[0]||0,a=m.y1||m[1]||0,o=m.x2||m[2]||0,m=m.y2||m[3]||0,t='angle="'+(90-R.atan((m-a)/(o-c))*180/Ma)+'"',s();
            else{
...
symbols:{
    ...
    rect:function(a,b,c,d,e){
        var f=a+c,g=b+d,h;
        !t(e)||
        !e.r?f=Ga.prototype.symbols.square.apply(0,arguments):(h=C(e.r,c,d),f=["M",a+h,b,"L",f-h,b,"wa",f-2*h,b,f,b+2*h,f-h,b,f,b+h,"L",f,g-h,"wa",f-2*h,g-2*h,f,g,f,g-h,f-h,g,"L",a+h,g,"wa",a,g-2*h,a+2*h,g,a+h,g,a,g-h,"L",a,b+h,"wa",a,b,a+2*h,b+2*h,a,b+h,a+h,b,"x","e"]);
        return f
        }
    }
};
...
tb.prototype={
    render:function(){
        ...
        if(o)q?o.animate({
            ...
            },s)e(n);if(f&&t(f.text)&&q&&q.length&&b.width>0&&b.height>0){
            f=x({
                align:c&&k&&"center",
                x:c?!k&&4:10,
...
Mb.prototype={
    ...
    setOffset:function(a,b){
        var c=this.axis,d=c.chart,e=d.inverted,f=this.isNegative,g=c.translate(this.percent?100:this.total,0,0,0,1),c=c.translate(0),c=O(g-
            c),h=d.xAxis[0].translate(this.x)+a,i=d.plotHeight,f={
            x:e?f?g:g-c:h,
...
init:function(a,b){
    ...
    if(a.inverted&&c&&this.reversed===v)this.reversed=
        !0;
    this.removePlotLine=this.removePlotBand=this.removePlotBandOrLine;
    for(f in d)J(this,f,d[f]);if(this.isLog)this.val2lin=na,this.lin2val=fa
        },
...
defaultLabelFormatter:function(){
    ...
    else if(f&&a>=1E3)for(;f--&&g===v;)c=Math.pow(1E3,f+1),a>=c&&e[f]!==null&&(g=za(b/c,-1)+e[f]);
    ...
    n(a.series,function(c){
        if(c.visible||!b.options.chart.ignoreHiddenSeries){
            ...
            if(a.isXAxis){
                ...
                if(t(e))if(a.dataMin>=e)a.dataMin=e,a.ignoreMinPadding=!0;
...
getLogTickPositions:function(a,b,c,d){
    ...
    else if(a>=0.08)for(var f=P(b),h,i,j,k,l,e=a>0.3?[1,2,4]:a>0.15?[1,2,4,6,8]:[1,2,3,4,5,6,7,8,9];f<
        c+1&&!l;f++){
        i=e.length;
...
setScale:function(){
    ...
    if(e||d||this.isLinked||this.forceRedraw||this.userMin!==this.oldUserMin||this.userMax!==this.oldUserMax){
        if(!this.isXAxis)for(b in a)for(c in a[b])a[b][c].total=null;this.forceRedraw=!1;
...
setExtremes:function(a,b,c,d,e){
    var f=this,g=f.chart,c=p(c,!0),e=s(e,{
        min:a,
        max:b
    });
...
destroy:function(a){
    ...
    a||ba(b);
    for(d in c)Ja(c[d]),
        c[d]=null;n([b.ticks,b.minorTicks,b.alternateBands],function(a){
        Ja(a)
        });
...
vb.prototype={
    ...
    runPointActions:function(a){
        ...
        if(d&&this.options.tooltip.shared&&(!g||!g.noSharedTooltip)){
            e=[];
            h=c.length;
            for(i=0;i<h;i++)if(c[i].visible&&c[i].options.enableMouseTracking!==!1&&!c[i].noSharedTooltip&&c[i].tooltipPoints.length&&(b=c[i].tooltipPoints[k],b.series))b._dist=O(k-b.clientX),j=C(j,b._dist),e.push(b);for(h=e.length;h--;)e[h]._dist>j&&e.splice(h,1);
            if(e.length&&e[0].clientX!==this.hoverX)d.refresh(e,a),this.hoverX=e[0].clientX
                }
                if(g&&g.tracker){
            if((b=g.tooltipPoints[k])&&b!==f)b.onMouseOver(a)
                }else d&&d.followPointer&&
            !d.isHidden&&(a=d.getAnchor([{}],a),d.updatePosition({
                plotX:a[0],
                plotY:a[1]
                }))
            },
...
xb.prototype={
    ...
    if(e)for(g in e)J(f,g,e[g]);f.xAxis=[];
    f.yAxis=[];
...
generatePoints:function(){
    ...
    if(b&&(g!==(c=b.length)||j))for(m=0;m<c;m++)if(m===
        h&&!j&&(m+=g),b[m])b[m].destroyElements(),b[m].plotX=v;this.data=b;
    this.points=l
    },
...
getExtremes:function(){
    ...
    if(!t(d)||!t(c)){
        for(b=0;b<i;b++)if(o=g[b],f=h[b],e=f!==null&&f!==v&&(!a.isLog||f.length||f>0),o=this.getExtremesFromAll||this.cropped||(g[b+1]||o)>=m&&(g[b-1]||o)<=l,e&&o)if(e=f.length)for(;e--;)f[e]!==null&&(j[k++]=f[e]);else j[k++]=f;d=p(d,Ia(j));
        c=p(c,ua(j))
        }
        this.dataMin=d;
    this.dataMax=c
...
drawPoints:function(){
    ...
    if(l.enabled||this._hasPointMarkers)for(f=b.length;f--;)if(g=b[f],d=P(g.plotX),e=g.plotY,k=g.graphic,i=g.marker||{},a=l.enabled&&i.enabled===v||i.enabled,m=c.isInsidePlot(u(d),e,c.inverted),a&&e!==v&&!isNaN(e)&&g.y!==null)if(a=g.pointAttr[g.selected?"select":""],h=a.r,i=p(i.symbol,this.symbol),j=i.indexOf("url")===0,k)k.attr({
...
getAttribs:function(){
    ...
    for(g=i.length;g--;){
        ...
        if(h.options)for(o in l)t(c[l[o]])&&(f=!0);if(f){
            c=c||{};
    ...
    if(b.enabled||a._hasPointLabels)a.dlProcessOptions&&a.dlProcessOptions(b),g=a.plotGroup("dataLabelsGroup","data-labels",a.visible?"visible":"hidden",b.zIndex||6),e=b,n(c,function(c){
        ...
        if(j&&!i)c.dataLabel=j.destroy();
            ...
            if(j)if(t(f))j.attr({
                ...
                for(l in j)j[l]===v&&delete j[l];j=c.dataLabel=a.chart.renderer[i?"text":"label"](f,0,-999,null,null,null,b.useHTML).attr(j).css(b.style).add(g).shadow(b.shadow)
                }
                j&&
...
alignDataLabel:function(a,b,c,d,e){
    var f=this.chart,g=f.inverted,h=p(a.plotX,-999),i=p(a.plotY,-999),a=b.getBBox(),d=s({
        x:g?f.plotWidth-i:h,
...
F=ha(Q,{
    ...
    getSegments:function(){
        ...
        if(this.options.stacking&&!this.cropped){
            ...
            for(m in f)c.push(+m);c.sort(function(a,b){
                return a-b
                });
            n(c,function(a){
...
drawDataLabels:function(){
    var a=this,b=a.data,c,d=a.chart,e=a.options.dataLabels,f=p(e.connectorPadding,10),g=p(e.connectorWidth,1),h=d.plotWidth,d=d.plotHeight,i,j,k=p(e.softConnector,!0),l=e.distance,m=a.center,o=m[2]/2,q=m[1],t=l>0,s,w,v,x,A=[[],[]],z,E,G,I,B,C=[0,0,0,0],H=function(a,b){
        return b.y-a.y
        },K=function(a,b){
        a.sort(function(a,c){
